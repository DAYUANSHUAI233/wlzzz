...
# Vercel Serverless Function 主入口 - 使用 BaseHTTPRequestHandler 格式
    ...
    def do_GET(self):
        ...
        try:
            if path == '/' or path == '':
                ...
                result = handle_plates()

            elif path.startswith('/stocks/'):
                plate_id = path.split('/')[-1]
                result = handle_stocks(plate_id)

            elif path == '/market/updown':
                result = handle_market_updown()

            elif path == '/market/retrace':
                result = handle_market_retrace()

            elif path == '/market/volume':
                result = handle_market_volume()

            elif path == '/market/mood':
                result = handle_market_mood()

            elif path.startswith('/market/loss/'):
                market_date = path.split('/')[-1]
                result = handle_market_loss(market_date)

            elif path.startswith('/market/trend/'):
                market_date = path.split('/')[-1]
                result = handle_market_trend(market_date)

            elif path == '/market/days':
                result = handle_market_days()

            elif path == '/all':
                result = handle_all()

            else:
                result = json_response({'error': 'Not Found'}, 404)

            self.wfile.write(json.dumps(result).encode())

        except Exception as e:
            result = json_response({'error': str(e)}, 500)
            self.wfile.write(json.dumps(result).encode())
